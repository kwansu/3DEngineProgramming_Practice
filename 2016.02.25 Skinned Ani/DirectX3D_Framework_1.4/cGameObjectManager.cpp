...
void cGameObjectManager::Setup()
{
	...
	m_pSkinning = new cSkinningMesh;

	m_pSkinning->Load("zealot.X", "./Zealot/");
...
void cGameObjectManager::InputProcess(iInputProcessor * pGenerator)
{
	...
	if (g_pInput->IsKeyDown('1'))
		m_pSkinning->SetAnimaitonTrack(0);

	if (g_pInput->IsKeyDown('2'))
		m_pSkinning->SetAnimaitonTrack(1);

	if (g_pInput->IsKeyDown('3'))
		m_pSkinning->SetAnimaitonTrack(2);

	if (g_pInput->IsKeyDown('4'))
		m_pSkinning->SetAnimaitonTrack(3);

	if (g_pInput->IsKeyDown('5'))
		m_pSkinning->SetAnimaitonTrack(4);

	if (g_pInput->IsKeyDown('6'))
		m_pSkinning->SetAnimaitonTrack(5);

	if (g_pInput->IsKeyDown('7'))
		m_pSkinning->SetAnimaitonTrack(6);

	if (g_pInput->IsKeyDown('8'))
		m_pSkinning->SetAnimaitonTrack(7);

	//static int i = 0;
	//if (g_pInput->IsKeyDown('5'))
	//	m_pSkinning->SetAnimaitonTrack(i++);
