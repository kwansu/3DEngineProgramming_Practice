...
void ASELoader::SetGeometry(ifstream* pIfASE)
{
	...
	while (nRefCount)
	{
		...
		if (!strcmp(token, "MESH_FACENORMAL"))
		{
			...
			m_aVertex[vertexIndex].n = vNor[0];
			m_aVertex[vertexIndex + 1].n = vNor[2];
			m_aVertex[vertexIndex + 2].n = vNor[1];
			continue;
		}
		...
		if (!strcmp(token, "MESH_FACE"))
		{
			...
			m_aVertex.push_back(aPos[nWinding[0]]);
			m_aVertex.push_back(aPos[nWinding[2]]);
			m_aVertex.push_back(aPos[nWinding[1]]);
			continue;
		}
		...
		if (!strcmp(token, "MESH_TFACE"))
		{
			...
			m_aVertex[i].t = aTex[nTexI[0]];
			m_aVertex[i + 1].t = aTex[nTexI[2]];
			m_aVertex[i + 2].t = aTex[nTexI[1]];
			continue;
		}
	...
	g_pDevice->CreateVertexBuffer(
		m_aVertex.size() * sizeof(Vertex_PNT),
		0,
		Vertex_PNT::FVF,
		D3DPOOL_MANAGED,
		&pGeo->m_pVertexBuffer,
		0);

	Vertex_PNT* pv;
	pGeo->m_pVertexBuffer->Lock(0, 0, (void**)&pv, 0);

	for (int i = 0; i < m_aVertex.size(); ++i)
	{
		*pv++ = m_aVertex[i];
	}
	pGeo->m_pVertexBuffer->Unlock();
	pGeo->m_nVertexSize = m_aVertex.size();
...
void ASELoader::Release()
{
	...
	for each(auto pG in m_mapGeometry)
		SafeRelease(pG.second.m_pVertexBuffer);
}
